import { MapPin, Users, Bell, User, Trophy } from 'lucide-react';
...
export const BottomNav = () => {
  ...
  const navItems = [
    ...
    { path: '/steps', icon: Trophy, label: 'Шаги' },
            ...
            >
              <div
                className={`flex items-center justify-center w-10 h-9 rounded-2xl transition-all duration-200 ${
                  active
                    ? path === '/steps'
                      ? 'bg-amber-400/15 text-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.2)]'
              ...
              <span className={`text-[9px] font-semibold mt-0.5 transition-colors ${
                active
                  ? path === '/steps' ? 'text-amber-400' : 'text-accent'
                  : 'text-white/40'
              }`}>
                {label}
              </span>
